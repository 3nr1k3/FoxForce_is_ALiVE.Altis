...
private ["_bloodVolume", "_painLevel", "_bleedingState", "_ivBags", "_bloodLossPct", "_bloodLossVol", "_painPct"];

private ["_patient", "_text", "_textBlood", "_textBleeding", "_textPain", "_textBags"];
...
if (!alive _unit) then
{
	hintSilent (format ["%1 is dead, rest in peace", _patient]);
}
else
{
	...
	_text = (format ["%1's health status<br/><br/>%2<br/>%3<br/>%4<br/>%5<br/>%6", _patient, _textBleeding, _textBlood, _textPain, _textBags]);
