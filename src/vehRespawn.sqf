...
_lazyEH =
{
	...
	[_target] spawn
	{
		...
		{
			if (({isPlayer _x} count (_target nearEntities ["Man", _rad])) > 0) then
			{
...
if (isServer) then
{
	...
	while {true} do
	{
		...
		if ((alive _veh) && {canMove _veh} && {{alive _x} count crew _veh isEqualTo 0} && (({isPlayer _x} count (_veh nearEntities ["Man", 1250])) isEqualTo 0)) then
		{
			_abandoned = true;
