...
_posOK	= false;
_msg	= "Find druglab and destroy it";
...
while {!_posOK} do
{
	_marker = (FF7_OP_markersObj call BIS_fnc_selectRandom);

	//diag_log format ["Try position in: %1", (markerText _marker)];

	if (((getMarkerPos _marker) distance FF7_OP_gatherPos) > 1000) then
	{
...
[
	"handleDamage",
	{
		params ["_target", "_part", "_damage", "_inflictor", "_ammo", "_pindex"];
...
/*
// ---------- Spawn gastank
_objGastank	= createVehicle ["Land_GasTank_01_khaki_F", [0, 0, 0], [], 0, "NONE"];
_relPos		= (_objTent modelToWorld [-0.97168,2.63403,0]);

_objGastank setDir 5.2286 + _rndDir;
_objGastank setPos [(_relPos select 0), (_relPos select 1), 0.813];
_objGastank setVectorUp [0,0,1];
_objGastank enableSimulation false;
_objGastank allowDamage false;
_objGastank setVariable ["gc_persist", true];

[_objGastank] call FF7_fnc_addToCurator;

_objSpawned = _objSpawned + [_objGastank];

GlobalChargeObj = _objGastank;

[
	_objGastank,
	"<img size='2' image='img\icon\charge.paa' /> <t size='1.3' color='#dd1111'>Plant charge</t>",
	{[] remoteExec ["OPS_fnc_setCharge", 2, false]},
	[],
	99,
	true,
	true,
