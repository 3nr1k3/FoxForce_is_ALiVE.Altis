private ["_posOK", "_marker", "_location", "_gatherPos", "_faction", "_facSide", "_objPos", "_fuzzyPos", "_msg", "_radius"];
...
while {!_posOK} do
{
	_marker = (FF7_OP_markersTown call BIS_fnc_selectRandom);

	if (((getMarkerPos _marker) distance FF7_OP_missionPos) > 1000) then
	{
		_posOK = true;
	};
};
...
_objPos = [_gatherPos, _radius] call OPS_fnc_findBuildingAndPos;

_faction = [_objPos] call OPS_fnc_getFactionFromPos;
...
[_gatherPos, _radius, 10, _officerGrp] call OPS_fnc_genWaypoints;

_markers = [_gatherPos, _radius, _msg, "ColorOPFOR", "DiagGrid", "mil_warning"] call OPS_fnc_createMarker;

["IntelGather", format ["Locate %1 officer in %2 and extract intel from him.", _faction, _location]] remoteExec ["FF7_fnc_globalNotify", 0, false];
...
{
	["Completed", format ["%1 officer in %2 found and intel successfully extracted.", _faction, _location]] remoteExec ["FF7_fnc_globalNotify", 0, false];
...
if !(alive _officer) then
{
	...
	["Failed", format ["The officer in %1 has been killed, mission failed.", _location]] remoteExec ["FF7_fnc_globalNotify", 0, false];
};
